...
from Queue_.q_services import get_queues, create_queue, dequeue_appointment, delete_queue_record
...
@queue_endpoint.post("/dequeue",status_code=status.HTTP_200_OK)
def Remove_from_Queue(db:db_dependency):
    return dequeue_appointment(db=db)

@queue_endpoint.delete("/delete/{queueId:str}",status_code=status.HTTP_200_OK)
def Delete_queue_record(db:db_dependency,queueId:str):
    return delete_queue_record(db=db,queue_id=queueId)