...
def Fetch_all_docter_info(db:db_dependency,limit:int,offset:int,token:Annotated[str,Depends(oauth2_bear)]):
...
def Fetch_doctor_by_Id(db:db_dependency,doctor_id:str,token:Annotated[str,Depends(oauth2_bear)]):
    user_info = verify_jwt_access_token(jwt_token=token,db=db)
...
def Fetch_doctor_by_Id(db:db_dependency,user_id:str,token:Annotated[str,Depends(oauth2_bear)]):
    user_info = verify_jwt_access_token(jwt_token=token,db=db)
...
def Create_new_doctor_record(db:db_dependency,new_value:create_doctor,token:Annotated[str,Depends(oauth2_bear)]):
    user_info = verify_jwt_access_token(jwt_token=token,db=db)
...
def Edit_doctor_info(db:db_dependency,values:edit_doctor,token:Annotated[str,Depends(oauth2_bear)]):
    user_info = verify_jwt_access_token(jwt_token=token,db=db)
